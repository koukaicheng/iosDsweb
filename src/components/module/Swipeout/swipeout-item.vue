<template>
  ...
  >
    <!--this.isOpen= true-->
    <div class="vux-swipeout-button-box vux-swipeout-button-box-left"
    ...
    <div class="vux-swipeout-content"
         :style="styles"
...
export default {
  ...
  methods: {
    ...
    start(ev) {
      ...
      if (this.$parent.$options._componentTag === "swipeout") {
        const openItems = this.$parent.$children.filter(
          item => item.$data.styles.transform.indexOf("(0px, 0, 0)") === -1
        );
    ...
    move(ev) {
      ...
      if (
        (this.direction === "right" && this.distX > 0 && this.hasRightMenu) ||
        (this.direction === "left" && this.distX < 0 && this.hasLeftMenu)
      ) {
        this.valid = true;
        ev.preventDefault();
      }
    ...
    open(position = "right") {
      this.setOffset(
        position === "right" ? -this.rightMenuWidth : this.leftMenuWidth,
        true,
        true
      );
    },
